    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert(appdata_t v)
            {
                v2f o;
                o.uv = v.uv;
                o.fragPos = mul(unity_ObjectToWorld, float4(v.position, 1.0)).xyz;
                o.pos = UnityObjectToClipPos(o.fragPos);
                return o;
            }
            ...
            float snoise(float3 v)
            {
                ...
                float3 x3 = x0 - float3(D.z, D.z, D.z);
                ...
                float3 ns = n_ * float3(D.w, D.w, D.w) - float3(D.x, D.x, D.x);
                ...
                float4 x = x_ * ns.x + float4(ns.y, ns.y, ns.y, ns.y);
                float4 y = y_ * ns.x + float4(ns.y, ns.y, ns.y, ns.y);
            ...
            float4 frag(v2f i) : SV_Target
            {
                ...
                for (int j = 0; j < 5; j++)
                {
                    float noise = snoise(float3(i.uv * scale, _CustomTime * 0.1)) * 0.5 + 0.5;
                ...
                float2 starLocal = fract(starCoord) - float2(0.5, 0.5);
                float starRandom = fract(sin(dot(starId, float2(12.9898, 78.233))) * 43758.5453);
                float star = 1.0 - step(starRandom, 0.998);
                star *= 1.0 - length(starLocal) * 2.0;
                color += float3(1.0, 1.0, 1.0) * max(star, 0.0) * 5.0;
